    ...
    public override void UseSkill(Vector2 direction, Vector2 playerPosition)
    {
        ...
        {   
            ...
            for (int i = 0; i < cols.Length; i++)
            {
                ...
                base.multiActions[i].Add(() => Hit(receiver, skillDamage, skillNode.skillData.SkillHitCount));
                base.multiActions[i].Add(() => RemoveTriggerModuleList(i));

                if (base.triggerModules[i].enabled)
                {
                    base.triggerModules[i].AddCollider(cols[i]);
                    base.interactions[i].ReceiveAction(multiActions[i]);
                }
            }   
        }
